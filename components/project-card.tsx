...
export default function ProjectCard({ project }: { project: Project }) {
  const CardContent = (
    <div className="group bg-background rounded-lg overflow-hidden border border-border hover:border-primary smooth-transition h-full cursor-pointer">
      <div className="relative h-60 overflow-hidden bg-card">
        <img src={project.image || '/placeholder.svg'} alt={project.title} className="w-full h-full object-cover group-hover:scale-110 smooth-transition" />
        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 smooth-transition" />
      </div>

      <div className="p-6">
        <div className="flex items-start justify-between mb-2">
          <span className="inline-block px-3 py-1 text-xs font-medium bg-primary/10 text-primary rounded-full">{project.category}</span>
  ...
  // Jika ada link → buat dapat diklik
  if (project.link) {
  ...
  // Kalau tidak ada link → tetap tampil biasa
  return CardContent;
}
